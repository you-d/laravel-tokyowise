...
class HomeController extends BaseController {
	...
	public function home() {
		...
		if ($this->enableCaching && Cache::has('home-headlineEntries')) {
		...
		if ($this->enableCaching && Cache::has('home-featureCategory')) {
		...
		if ($this->enableCaching && Cache::has('home-featurePosts')) {
	...
	private function getHeadlineEntry($headlinePageType, $headlineRecordId, &$headlineEntries) {
		switch ($headlinePageType) {
			case "features":
				...
				$urlPath = url("/features/" . $entry->post_id);
			...
			case "rensai":
				...
				$urlPath = url("/rensai/" . $entry->category_id . '/' . $entry->post_id);
			...
			case "gadgets":
				...
				$urlPath = url("/gadgets/no" . $entry->id);
